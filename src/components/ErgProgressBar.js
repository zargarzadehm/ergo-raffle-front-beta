...
const ErgProgressBar = ({ raffle }) => {
    const fillPercent = () => {
        return (parseFloat((!isNaN(raffle.erg) ? (raffle.erg/staticText.ERG_SCALE)/(raffle.goal/staticText.ERG_SCALE) : (((raffle && raffle.ticket && raffle.ticket.erg)) / raffle.goal)) * 100))
    }
