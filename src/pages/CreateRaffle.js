...
import { postRaffle } from "../service/raffle.service";
...
function CreateRaffle() {
  ...
  const [formParams, setFormParams] = useState({ address: '', walletAddress: (window.localStorage.getItem('wallet') !== null) ? window.atob(window.localStorage.getItem('wallet')) : '', files: [], ergGoal: '', ticketPrice: 0.25, ticketPercent: '', deadline: '', description: '' })
  ...
  const submitRaffleCreate = () => {
    context.setIsDonation(false);
    setIsTermsAccepted(true);
    postRaffle(formParams.raffleName, formParams.address, formParams.walletAddress, formParams.files, parseInt(formParams.ergGoal) * staticText.ERG_SCALE, parseInt((formParams.ticketPrice) * staticText.ERG_SCALE), parseFloat(formParams.ticketPercent), parseFloat(formParams.deadline), formParams.description, response).then(
      ({ data }) => {
        setTimeout(()=> {
